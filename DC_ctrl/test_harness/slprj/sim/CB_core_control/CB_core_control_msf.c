...
#define S_FUNCTION_NAME CB_core_control_msf
#endif
...
#endif
static void mdlInitializeConditions ( SimStruct * S ) { bh3neqetwem * dw = (
bh3neqetwem * ) ssGetDWork ( S , 0 ) ; jwkq2zcfvb ( & ( dw -> rtm ) , & ( dw
-> rtdw ) ) ; } static void mdlReset ( SimStruct * S ) { bh3neqetwem * dw = (
bh3neqetwem * ) ssGetDWork ( S , 0 ) ; fviyn2acc5 ( & ( dw -> rtm ) , & ( dw
-> rtdw ) ) ; } static void mdlPeriodicOutputUpdate ( SimStruct * S , int_T
tid ) { bh3neqetwem * dw = ( bh3neqetwem * ) ssGetDWork ( S , 0 ) ; real32_T
const * i_bnrqzarby0 = ( real32_T * ) ssGetInputPortSignal ( S , 0 ) ;
real32_T const * i_csha00ez10 = ( real32_T * ) ssGetInputPortSignal ( S , 1 )
; boolean_T const * i_duymj3etqb = ( boolean_T * ) ssGetInputPortSignal ( S ,
2 ) ; real32_T * o_B_2_1 = ( real32_T * ) ssGetOutputPortSignal ( S , 0 ) ;
real32_T * o_B_2_2 = ( real32_T * ) ssGetOutputPortSignal ( S , 1 ) ; if (
tid == 0 ) { CB_core_control ( i_bnrqzarby0 , i_csha00ez10 , i_duymj3etqb ,
o_B_2_1 , o_B_2_2 , & ( dw -> rtb ) , & ( dw -> rtdw ) ) ; idvm0hwalp ( & (
...
mr_CB_core_control_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S ,
...
mr_CB_core_control_MdlInfoRegFcn ( S , "CB_core_control" , & retVal ) ; if (
! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( bh3neqetwem ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { bh3neqetwem * dw = (
bh3neqetwem * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } m3e4cqcnxy
...
-> mdlInfo -> genericFcn ; } }
#define MDL_START
static void mdlStart ( SimStruct * S ) { bh3neqetwem * dw = ( bh3neqetwem * )
ssGetDWork ( S , 0 ) ; mp1ci1jqga ( & ( dw -> rtb ) ) ; } static void
mdlOutputs ( SimStruct * S , int_T tid ) { bh3neqetwem * dw = ( bh3neqetwem *
) ssGetDWork ( S , 0 ) ; real32_T const * i_bnrqzarby0 = ( real32_T * )
ssGetInputPortSignal ( S , 0 ) ; real32_T const * i_csha00ez10 = ( real32_T *
) ssGetInputPortSignal ( S , 1 ) ; boolean_T const * i_duymj3etqb = (
...
PARAMETER_TUNING_TID ) { CB_core_controlTID1 ( & ( dw -> rtb ) ) ; } if ( tid
!= CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { CB_core_control (
i_bnrqzarby0 , i_csha00ez10 , i_duymj3etqb , o_B_2_1 , o_B_2_2 , & ( dw ->
rtb ) , & ( dw -> rtdw ) ) ; } }
#define MDL_UPDATE
static void mdlUpdate ( SimStruct * S , int_T tid ) { bh3neqetwem * dw = (
bh3neqetwem * ) ssGetDWork ( S , 0 ) ; idvm0hwalp ( & ( dw -> rtb ) , & ( dw
-> rtdw ) ) ; return ; } static void mdlTerminate ( SimStruct * S ) {
bh3neqetwem * dw = ( bh3neqetwem * ) ssGetDWork ( S , 0 ) ; giuyfjohpx ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_CB_core_control_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_CB_core_control_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss
...
mr_CB_core_control_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss
